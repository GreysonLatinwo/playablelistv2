...
provider.enable().then(result => { console.log(result); });
...
const currplaylist = (): Node => (
    <View style={styles.container}>
        {playlist.map(({ songID, trackName, trackURI, albumImage, artist }) => {
            return (
                <React.Fragment key={songID}>
                    <View style={styles.separator} />
                    <TouchableOpacity
                        accessibilityRole={'button'}
                        onPress={() => openURLInBrowser('https://open.spotify.com/embed/track/' + trackURI.split(':')[2])}
